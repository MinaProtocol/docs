...
In this section, we'll connect to the `Devnet` network. Let's first start up the node so that we can begin issuing commands.
...
The first step in connecting to `Devnet` is installing the specific daemon version needed. Follow the instructions for your operating system below.
...
// TODO: What should the commit message be here?
...
1. Download the genesis ledger that will be needed for `Devnet`
```
  wget -O config.json https://raw.githubusercontent.com/MinaProtocol/Mina/devnet/automation/terraform/testnets/devnet/genesis_ledger.json
``` 

2. Run the daemon with the specified genesis file we just downloaded
```
  coda daemon -generate-genesis-proof true --peer-list-url https://storage.googleapis.com/seed-lists/devnet_seeds.txt -config-file config.json
```
...
gcr.io/o1labs-192920/coda-daemon-baked:0.4.2-245a3f7-devnet-4744658 \
daemon \
