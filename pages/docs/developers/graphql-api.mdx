...
<Alert>The sandbox does **NOT** connect you to a live network.

The memo field returned for a transaction is [Base58Check encoded](https://en.bitcoin.it/wiki/Base58Check_encoding).

</Alert>

To retrieve the details of an accountfor the primary mina [token](/docs/architecture/tokens):
...
:::info
Consult the [GraphQL schema](/graphql-docs/mutation.doc.html) for all available mutations.
:::
...
A GraphQL [subscription](h/graphql-docs/subscription.doc.html) allows a GraphQL client to have data pushed to it when an event occurs. In Mina, there are subscriptions for:
