...
This documentation is for the **beta** release. The commands and APIs may change before the initial release. Last updated for version `0.0.17-beta8`.
...
mkdir -p ~/.coda_config
cp /tmp/s3_cache_dir/config_9652f8e.json ~/.coda_config/daemon.json
```
...
Check that daemon installed correctly by running `coda version`. The output should read `Commit 9652f8ee092ea77e29f5ab49fa0a295e36743e8b on branch HEAD`.
...
Now that you've installed Mina and configured your network, let's move on to the next step - [generating a keypair](/docs/keypair)!
