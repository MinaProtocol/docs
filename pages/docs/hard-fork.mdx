...
If you are running an archive node, you will need to update the postgres database. We have included an automated script that will handle all the steps. Simply download [this sql script](https://github.com/MinaProtocol/mina/blob/develop-until-4.1-hardfork/scripts/archive/update_schema.sql) and execute it on the machine running the archive node.

First make sure your archive node is not running. Then download and execute the script.
...
Now you cam safely start the latest archive node.
