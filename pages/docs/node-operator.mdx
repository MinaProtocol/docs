...
export default Page({ title: "Tips for Node Operators" });
...
First make sure you've unlocked your account:
    ...
    coda account unlock -public-key $MINA_PUBLIC_KEY
...
Either way, your logs will be saved to `~/.coda-config/mina.log` and configuration can be specified in the [`daemon.json`](#using-daemonjson-to-configure-your-mina-daemon) file.
...
By creating a file at `~/.coda-config/daemon.json`, you can configure your mina daemon without needing to provide options on the commandline. This can be useful for running mina as a service as well as just for avoiding repetitive typing!
