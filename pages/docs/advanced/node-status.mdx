...
This sidecar will need to communicate with a running daemon, so you will first need to start your daemon. You will need to run the daemon with the flag `--limited-graphql-port 3095` in order for the sidecar to be able to communicate with the daemon. Further instructions can be found on the [connecting page](/docs/connecting).
...
Next you will need to start the daemon with `--network mina-network` passed to docker and a new flag `--limited-graphql-port 3095` passed to the daemon.
