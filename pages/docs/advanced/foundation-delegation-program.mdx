...
# Participation in Foundation Delegation Program
...
This should include all details that a block producer receiving Foundation/O(1) (let's call it the "token provider") delegations needs to understand to be compliant and maintain themselves as eligible for this delegation.
...
When participating in the Foundation Delegation Program you will also need to run the uptime sidecar, so please following [these instructions](Node Status Reporting) and set it up alongside your block producer daemon.

TODO: Attach the contents of CSVs that Ben has provided containing the list of addresses for Foundation and O(1)
