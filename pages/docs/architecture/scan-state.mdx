...
![Block 3](/static/img/docsImages/scan-state/tECFm3I.png)
...
![Block 4](/static/img/docsImages/scan-state/76R2bpU.png)
...
![Block 5](/static/img/docsImages/scan-state/QaqfbXG.png)
...
![Block 6](/static/img/docsImages/scan-state/y6dM2FT.png)
...
![Block 7](/static/img/docsImages/scan-state/RY8umxW.png)
...
![Emit proof](/static/img/docsImages/scan-state/hQvFVfp.png)
...
![Block 8](/static/img/docsImages/scan-state/A3o18oN.png)
...
![Block 11](/static/img/docsImages/scan-state/FaVTmWD.png)

<Alert>

View the contents of the scan state using the `coda advanced snark-job-list` command.

</Alert>

### Integration with the Snark Pool

Newly added jobs to the scan state are pending jobs for snark workers to complete.  Snark workers complete the required transaction SNARK proofs, submitting bids for their completed work. When a node receives and validates the completed work, they will add to their local snark pool if it is valid and the lowest fee for the required work. The work will also be gossiped to other peers in the network.
...
When a block producer includes completed proofs into a block to offset any transactions they add, they may purchase the corresponding work from the snark pool. For example, continuing the example above, consider the next block (12). If the block producer wants to add two transactions (which would include a coinbase and fee transfer transaction), they need to purchase two completed snark work corresponding to the 4 (`B9` and `B10`) proofs from the second tree (each snark work includes two _workIds_).

During the time the block is generated, the snark pool may include completed work in addition to the best bids for both jobs (0.025 and 0.165, respectively, in the example).

<img alt="Submitted work" src="/static/img/docsImages/scan-state/fDBZog9.png" width="550" />
...
The current snark pool is visible via [GraphQL](/docs/developers/graphql-api) or the CLI via the `coda advanced snark-pool` command.
