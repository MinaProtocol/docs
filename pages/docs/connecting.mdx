...
sudo apt-get update
sudo apt-get install -y curl mina-testnet-postake-medium-curves=0.2.0-efc44df --allow-downgrades
```
...
For Testworld, we are sending out pre-generated keypairs to every participant. Once you recieve your keypair, download it onto your computer and then unzip it and move the contents into the `~/keys` folder.
