...
Check that daemon installed correctly by running `coda version`. The output should read `Commit [DIRTY]31219a6f16a25d7b8a73d884ee1f14585dc28a3d on branch HEAD`.
...
sudo apt-get update
sudo apt-get install -y curl mina-testnet-postake-medium-curves=0.0.17-beta8-9652f8e --allow-downgrades
```

Check that daemon installed correctly by running `coda version`. The output should read `Commit [DIRTY]9652f8ee092ea77e29f5ab49fa0a295e36743e8b on branch HEAD`.

## Get the network information

To connect to the network, we need an extra file: the <em>peer list</em>.
...
Due to the limited number of spots available, you'll first need to be selected as a participant to get the latest peer list. The inital participants have already been selected, but you can sign up for the waitlist [here](https://bit.ly/TestnetSignUp) in case a spot opens up.
