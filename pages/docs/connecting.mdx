...
<Alert kind="warning">
  ...
  For `release 0.0.16-beta7` the MacOS has not been tested and as such will be temporarily unsupported. Instead please follow the [Docker](#docker) instructions below.
...
Check that process installed correctly by running `coda -help`.
