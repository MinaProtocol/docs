...
sudo apt-get update
sudo apt-get install -y curl mina-testnet-postake-medium-curves=0.0.17-beta6-e2db96a --allow-downgrades
```

Check that daemon installed correctly by running `coda version`. The output should read `Commit [DIRTY]e2db96a380d5e29050f888bb8233b8de0312db75 on branch HEAD`.
