...
Check that daemon installed correctly by running `coda version`. The output should read `Commit [DIRTY]efc44df72dfc8fd1dab7ba5992e2566c10792fa3 on branch HEAD`.

## Setup Keypair

For Testworld, we are sending out pre-generated keypairs to every participant. Once you recieve the link to claim your keypair, download it onto your computer.
...
```
CODA_PRIVKEY_PASS="password from email" coda daemon -peer-list-file ~/peers.txt -block-producer-key ~/keys/my-wallet -generate-genesis-proof true -file-log-level Info
```
