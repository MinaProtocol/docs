...
Use [Homebrew](https://brew.sh/) to update the `mina` package.

```
brew upgrade mina
```

Check that the output of the `coda -version` command is `Commit 4395e91 on branch master`.
...
sudo apt-get install -y mina-testnet-postake-medium-curves=0.0.16-beta7+-hard-fork-test-11-12-2020-069333
```
...
To connect to the network, we need two files: <em>peer list</em> and <em>genesis config</em>. 

### Peer list

A peer list contains a list of node addresses that your node can initially connect to while joining the network.

Due to the limited number of spots available, you'll first need to be selected as a participant to get the latest peer list. The inital participants have already been selected, but you can sign up for the waitlist [here](http://bit.ly/StakingSignup) in case a spot opens up.

Once you're in, you'll receive an email with instructions on how to download latest peer list. Use the following command but replace `<PEER-LIST-URL>` with the URL provided in the email.

```
wget -O ~/mina-peers.txt <PEER-LIST-URL>
```

### Genesis config

The genesis config contains the genesis timestamp and ledger, which together consist of the initial state of the network. For the genesis config we can simply download it using the following command.

```
wget -O ~/mina-daemon.json https://raw.githubusercontent.com/MinaProtocol/coda-automation/master/terraform/testnets/pickles-public/genesis_ledger_fork_11-12-2020.json
```
...
The `-peer` arguments specified above refer to the seed peer address - this is the initial peer we will connect to on the network. Since Mina is a [peer-to-peer](/docs/glossary#peer-to-peer) protocol, there is no single centralized server we rely on.
...
The `-background` flag ensures that we don't accidentally kill the process.
...
Now we can access this everywhere as `$CODA_PUBLIC_KEY` -- check if it saved properly by trying `echo $CODA_PUBLIC_KEY`.
...
Now that we have a fully synced node, we can learn about some of the more advanced features of the daemon in [Tips for Node Operators](/docs/node-operators).
