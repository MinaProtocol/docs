...
The output of `coda client status` outputs similar to the following:
...
~~Just leave your node running as it should be operating correctly but incorrectly reporting the max observed block length. This should not affect your ability to produce blocks or send transactions.~~

### My node is stuck in catchup / the block height does not update

When the node is synced, **Block height** should be equal to **Max observed block length**, as seen in the output of `coda client status` and should match the current height of the network, as shown in the screenshot below:
...
What matters is that your current blockchain length, as seen in `coda client status`, is equal to the current network height. If your node is reporting it is synced at the wrong height, it should eventually enter the catchup phase to get the node to the correct height. You may occasionally switch between catchup and synced if the network sees any short forks it tries to catch up to.
...
~~If you are certain that you have the correct `peers.txt` file and still experiencing the issue, you can remove or move the existing `.coda-config` directory and restart the node, which should clear the error.~~ This error should have been resolved by a recent update, so ensure you are on the latest release and have the latest peers file.
...
* Run as a systemd service (see [the docs](https://minaprotocol.com/docs/connecting)).
* Run with Docker (see [the docs](https://minaprotocol.com/docs/connecting)).
...
See the requirements for the current testnet [here](https://minaprotocol.com/docs/getting-started). Some lower-tier VPS providers may have an issue producing blocks within the slot time. If you continue to experience missing blocks, you may wish to try on more powerful hardware.
...
When you generated a keypair and submitted it to participate in the testnet, you created a password. Use the password you created for the value of `CODA_PRIVKEY_PASS`.

### I get an error about ~/.coda-config/daemon.json missing?
...
See this [guide](https://mina.docs.casso.io/english/testnet-4.1/ubuntu-20.04.1-lts) to manually resolve the required dependencies. Alternatively, use a supported OS (Ubuntu 18.04/Debian 9) or use Docker.
...
On the current testnet, this is 128, which also includes the coinbase transaction as well as any fee transfers to pay snark workers.
...
See [this answer](https://hackmd.io/nzZFoUzVTiyHgezL2rpf1w?both#What-fee-should-I-use-to-send-a-transaction--My-transaction-is-stuck-as-pending). The transaction was likely stuck as pending and then eventually evicted from the transaction pool. Try sending again.
...
If the output of `coda accounts list` shows your account is locked, this means that you need to unlock it using your private key password in order to use it to send transactions. To unlock, simply use the `coda accounts unlock --public-key` command.
...
See [this answer](https://hackmd.io/@garethtdavies/rJg5G_BiD#Can-I-send-a-payment-before-I-am-synced), you will need to wait until you are synced if you do not have an existing balance in the ledger.

### I am running a block producer, but I don't see anything in coda accounts list?

If you started the daemon passing the `block-producer-key` flag, then you still need to import the account to the daemon in order to send a transaction. To do so, use the `coda accounts import -priv-key` command, passing in the location of the private key file.

## Block producer

### How do I run a block producer?

The methods to start a node [in the documentation](https://minaprotocol.com/docs/connecting) will run a block producer by default.

### How do I know if I am running a block producer

To check the status of your block producer, run `coda client status` and look for the lines **Block producers running**. You should see a value of 1 and also your public key.
...
A slot on the current testnet is every 3 mins, though not all slots should have a block produced, so on average, we would expect a block every 4 mins. However, not all the stake is online and active in producing blocks, and so not all slots will have a block, and sometimes there can be long delays between blocks. As more of the stake is online and staking, this situation improves.
...
See [this answer](https://hackmd.io/nzZFoUzVTiyHgezL2rpf1w#How-can-I-increase-my-chance-of-winning-a-block).

### Are orphaned blocks considered for the challenges?

Yes, during this testnet, orphaned blocks will be considered.
...
Sometimes high fees will be included in blocks, and this is a function of how snark workers select which work to complete and which work is required to be purchased by the block producer. You can read more about snark workers in these draft docs [here](https://hackmd.io/gHjzytCLT0qRUIGojZfxNQ).

By default, the work selection for a snark worker is random. You can change this by adding the `-work-selection seq` flag to the `coda daemon` command, which will work on jobs in the order required to be included from the scan state and will likely result in your snarks being included without a potentially lengthy delay.
...
Run `coda client set-snark-worker` to disable the snark worker. To enable again, pass your public key `coda client set-snark-worker -address <YOUR_PUBLIC_KEY>`.
...
`tail -f ~/.coda-config/coda.log`

### How many logs are kept / where are the logs located?

The `~/.coda-config` directory contains the Mina logs. This directory contains the following:

* `coda.log` - This file contains the latest logs of the daemon. Each log file is limited to 10 MiB in size and rotates through 50 log files. Rotated log files are named `coda.log.x` from `coda.log.0` to `coda.log.50`.
...
* `coda-prover.log` - This logs memory usage and batch size of the prover and is limited to 128 MiB and rotates via a single log file.
* `coda-verifier.log` - This logs memory usage and batch size of the verifier and is limited to 128 MiB and rotates via a single log file.
* `coda.version` - This file contains the Git SHA of the running daemon.

## Testnet Challenges

### I lost the password for my private key I submitted for the testnet?

If you lost the password for the private key, then there is nothing that can be done to recover it. If you were selected for the testnet, the ledger may have a balance associated with this public key, and it isn't possible to move this to a new key. You can still participate, however, by creating a new keypair and getting a node running and synced. You won't have a balance on the key, so you will not win any blocks but can participate in running a node and in the other community challenges such as running a snark worker.

Always keep a copy of your private key you generated (likely called `my-wallet` in a folder named `keys`) and the associated password!

### I completed a challenge, and the leaderboard has not been updated?

As per the message in the #leaderboard-qa channel on Discord, please be patient!

> We're still working on a fix for the leaderboard. We have recovered the block data for the service that went down, and we're in the process of restoring the Leaderboard service. You can expect some updates soon.

### I was on the waitlist, and now my balance displays 0?

If you do not have a balance for your private key, check that you are using the same key that you used to register for the testnet.

Not all waitlist participants have a balance, and if you are one of them, you can still sync a node and request funds from the faucet or other community members. Your node will need to be synced before any balance from the faucet or community members will be visible via `coda accounts list`. There may be additional stake redistributions at a later time, assuming you have a synced node.

### Do I need to send 50 tx to different addresses / to what address should I send?

You can send the transactions to any address. It doesn't matter what fees/amounts/addresses you use - just 50 transactions from your public key. The key has to be the one registered for the testnet as it is associated with your Discord ID.

### Is there a guide to completing the testnet challenges?

See [this guide](https://icohigh.gitbook.io/mina-node-testnet/) by community member @Ducca.

### Where can I find a list of all the current testnet challenges?

See the post [here](https://forums.minaprotocol.com/t/testnet-release-4-1-challenges/597).
