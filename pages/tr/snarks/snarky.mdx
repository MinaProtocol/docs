...
Burada **snarky** kullanımı ve zk-SNARK kullanımı için DSL ile ilgili dökümantasyonu ve bulabilirsiniz. Here you can find documentation on how to use **snarky**, a DSL for using zk-SNARKS, as well as **snarkyjs-crypto** a companion JavaScript library providing a suite of cryptographic primitives suitable for use with **snarky**.

## SNAPP Yapısı

SNAPP (yada snarkified uygulama) iki ana bölümden oluşur:

1. The definition of your zk-SNARK programınızın tanımı. Bu bölüm **snarky** ve özellikle <DocLink copy="snarky-evreni" url="/snarks/snarky-universe" /> standart bilgi bankası kullanılarak geliştirilir.
2. There are currently APIs for Node.js and OCaml/ReasonML for interacting with **snarky** programs in this way.
