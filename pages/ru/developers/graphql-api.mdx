...
Daemon Mina раскрывает [GraphQL API](https://graphql.org/), используемый для запроса информации и отправки команд работающей ноде. По умолчанию HTTP-сервер работает через порт `3085`, хотя эту конфигурацию можно изменить, передав флаг `-rest-port` команде запуска daemon.
