...
1. Установите последнюю версию mina. Если вы не обновились до последней версии daemon, перейдите <DocLink copy="назад к документации" url="/node-operators/getting-started" />, чтобы получить последнюю версию. Вы можете запустить код `mina -help`, чтобы проверить, как прошла установка.
