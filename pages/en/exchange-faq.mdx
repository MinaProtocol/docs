...
<DocLink
  copy="instructions for connecting to DevNet"
  url="/advanced/connecting-devnet"
/>
.
...
Archive node operators often choose to run

<DocLink copy="redundant archive nodes" url="/advanced/archive-redundancy" /> to
store block data to one or more locations of their choice (e.g. PostgreSQL, GCP,
local files, a logging service, etc) and to backfill any missed block data if needed.
...
This can be accomplished using a recursive query. See

<DocLink copy="Example #3" url="/advanced/archive-node#try-a-query" /> in the Archive
Node docs, for a full example.
