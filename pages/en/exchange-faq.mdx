...
Archive node operators often choose to run <DocLink copy="redundant archive nodes" url="/advanced/archive-redundancy" /> to store block data to one or more locations of their
...
This can be accomplished using a recursive query. See <DocLink copy="Example #3" url="/advanced/archive-node#try-a-query" /> in the Archive Node docs, for a full example.
