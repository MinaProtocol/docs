...
You can download all the software required to run a Mina node <DocLink copy="here" url="/getting-started#installation" />. Right now, we have builds that work on macOS, as well as Debian 9 and Ubuntu 18.04. You can also [build from source](https://github.com/minaprotocol/mina) or run via <DocLink copy="Docker" url="/using-mina/connecting#docker" /> for other unsupported systems.
...
Head over to the <DocLink copy="Docker" url="/using-mina/connecting#docker" /> <DocLink copy="Devnet page" url="/advanced/connecting-devnet" /> to learn more and get started.
...
<DocLink copy="See here" url="/using-mina/connecting#ubuntu-1804--debian-9-1" /> for information about using launchd or systemd with mina.
