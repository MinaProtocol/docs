...
You can download all the software required to run a Mina node <DocLink copy="here" url="/node-operators/getting-started#installation" />. Right now, we have builds that work on macOS, as well as Debian 9 and Ubuntu 18.04. You can also [build from source](https://github.com/minaprotocol/mina) or run via <DocLink copy="Docker" url="/node-operators/connecting#docker" /> for other unsupported systems.
...
Head over to the <DocLink copy="Docker" url="/node-operators/connecting#docker" /> <DocLink copy="Devnet page" url="/node-operators/connecting-devnet" /> to learn more and get started.
...
See the <DocLink copy="documentation for zkapps" url="/architecture/zkapps" />, Mina's Snarkified applications.
...
<DocLink copy="See here" url="/node-operators/connecting#ubuntu-1804--debian-9-1" /> for
