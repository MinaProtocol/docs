...
To generate a signed transaction offline if your private key is on a Ledger device, see <DocLink copy="Ledger Offline Mode" url="/node-operators/ledger-app-mina#offline-mode" />.
...
We are assuming in the rest of section that you have the Mina client installed on your system, if you do not have Mina installed please see the <DocLink copy="Getting Started" url="/node-operators/getting-started" />.
...
Run the following command to import your <DocLink copy="previously generated" url="/node-operators/keypair" /> keypair file:
...
Once you feel comfortable with the basics of creating an address, and sending & receiving mina, we can move on to the truly unique parts of the Mina network like <DocLink copy="participating in consensus and helping compress the blockchain" url="/node-operators/staking" />.
