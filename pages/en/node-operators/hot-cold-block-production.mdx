...
See our documentation page on <DocLink copy="generating keys" url="/node-operators/keypair" />. Note that as this is a hot wallet, you should not use the ledger or any other HSM for this keypair.
...
See our documentation page on <DocLink copy="generating keys" url="/node-operators/keypair" />. Note that as this is a cold wallet, you should prefer using the most secure mechanism for generating keys as you can preferably on a machine disconnected from the internet.
...
Follow the "Start up a node" instructions in the <DocLink copy="Connecting to the Network" url="/node-operators/connecting" /> page using your hot wallet key.
