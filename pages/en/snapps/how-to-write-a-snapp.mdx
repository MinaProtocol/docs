...
It takes the snapp account address (a public key) as its only argument:

```ts
let snappKey = PrivateKey.random();
let snappAddress = PublicKey.fromPrivateKey(snappKey);

let snapp = new SmartContract(snappAddress);
```

##### Deployment

There is one method that every smart contract needs: `deploy`. It describes how the snapp account
will be initialized when the snapp is deployed:
...
The deploy method needs one default parameter, the initial balance of the snapp account.
...
The method then and sets the new state to `x + 1` using `this.x.set()`.
