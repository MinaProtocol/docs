...
Snapp CLI makes it easy to follow recommended best practices by providing project scaffolding including dependencies such as SnarkyJS, a test framework (<a href="https://jestjs.io/">Jest</a>), code auto-formatting (<a href="https://prettier.io/
...
class HelloWorld extends SmartContract {
  ...
  constructor(address: PublicKey, initialBalance: UInt64, x: Field) {
    super(address);
...
The constructor needs two default parameters:

- The account address
- The initial balance of the snapp account
...
All state variables must be initialized within the constructor using `State.init()` --e.g.:
...
To update your snapp state or to send MINA, your smart contract will need one or more custom methods.
...
This is how you can fetch the current state of the snapp account from the chain.

The method then logs the current state, and sets the new state to `x + 1` using `this.x.set()`.
...
While the state of a snapp is **public**, method parameters are **private**.

When a smart contract method is called, its code will execute on the client and generate a proof, which is then sent to the network to update the chain.
...
Now that you've made it this far, you're now ready to start a project!

If you run into any questions or want to discuss snapps with other developers, join the [#snapp-developers](https://discord.gg/654BcVJf) channel on Discord. 

If you want to see all resources related to snapps, including community built snapps, check out the [Snapp Developer Resource Kit](https://github.com/o1-labs/snapp-resource-kit).