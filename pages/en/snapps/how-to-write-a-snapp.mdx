...
In the current design, the state can consist of at most 8 Fields.
...
The first two lines above are mandatory:
...
All state variables have to be initialized and wrapped with `State.init()`.

##### Methods and state updates

To update your snapp state and trigger token transfers, a smart contract needs custom methods.
SnarkyJS allows you to call one or more methods within one transaction.
...
This is how you can fetch the current state of a snapp account from the chain.
...
Let's modify the `increment()` method to take in a parameter:
...
If the assertion fails, SnarkyJS will throw an error and not submit the transaction containing the method call.
...
As we saw before, on-chain state can be managed inside smart contract methods with `.get()` and `.set()`.
...
<!-- _(Gregor will update this section)_ -->
