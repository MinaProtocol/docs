...
If you haven’t yet read the <DocLink copy="how zkApps work" url="/snapps/how-snapps-work" /> section, please read it first so that this section makes sense.
...
```ts
let zkKey = PrivateKey.random();
let zkAddress = PublicKey.fromPrivateKey(zkKey);

let zk = new SmartContract(zkAddress);
```
