...
- <DocLink copy="Parameterized records" url="/node-developers/style-guide/#parameterized-records" />
- <DocLink copy="Ppx deriving" url="/node-developers/style-guide/#ppx_deriving" />
- <DocLink copy="Stable.V1" url="/node-developers/style-guide/#stable-v1" />
- <DocLink copy="Property based tests" url="/node-developers/style-guide/#quickcheck-gen" />
- <DocLink copy="Typesafe Invariants" url="/node-developers/style-guide/#typesafe-invariants" />
- <DocLink copy="Unit Tests" url="/node-developers/style-guide/#unit-tests" />
...
This is the first time we see heavily functored code, so see <DocLink copy="functors" url="/node-developers/style-guide/#functors" /> if you're confused. This is also the first time we see custom SNARK circuit logic, see <DocLink copy="custom SNARK circuit logic" url="/node-developers/style-guide/#snark-checked" /> for more.
...
If it's important to prove your payment went through, you ask the receiver to start recording receipt chain hashes, and hand your payment payload over to the receiver. He can then check the top hash of their receipt chain to see if it includes your payload.
