...
docker run \
  ...
  -e RUN_DEMO=true \
  -e MINA_PRIVKEY_PASS='' \
  minaprotocol/mina-daemon:1.3.1-3e3abec-bullseye-devnet
...
You can use the Mina CLI to interact with the sandbox node. The following command opens a shell inside the docker container from where you can issue any of the available <DocLink copy="mina commands" url="/using-mina/cli-reference" />.
...
- <DocLink copy="Install Mina" url="/getting-started#installation" /> as usual and use many of the client commands. Since the daemon is already running in the container, you don't need to run `mina daemon`!
