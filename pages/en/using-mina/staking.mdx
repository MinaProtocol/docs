...
export default Page({ title: "Staking & Snarking | Mina Protocol", description: "Staking and snarking with Mina allows you to participate in consensus on the Mina network."  });
...
### Producing our own Blocks (Staking MINA)

We can try out staking with our MINA by restarting your mina daemon with the `-block-producer-pubkey` flag:

<DaemonCommandExample args={["--block-producer-pubkey $MINA_PUBLIC_KEY"]} />

Or in a .mina-env file:

<DaemonCommandExample args={["EXTRA_FLAGS=" --block-producer-pubkey $MINA_PUBLIC_KEY" "]} />
...
You'll notice that this command looks suspiciously like a <DocLink copy="payment" url="/using-mina/#make-a-payment" /> transaction. That's because a stake delegation is also a transaction! This is why we have to pay a small transaction fee in order to change this setting.
