...
sudo apt-get update
sudo apt-get install mina-generate-keypair=1.2.0-5b35b27
```

Check that keygen tool installed correctly by running `mina-generate-keypair -version`. The output should read `Commit 5b35b27df8e137ddbe88874b4efdcf4bb2cb4ae9 on branch master `.
...
```
cd ~
docker run  --interactive --tty --rm --volume $(pwd)/keys:/keys minaprotocol/mina-generate-keypair:1.2.0-5b35b27 --privkey-path /keys/my-wallet
```
