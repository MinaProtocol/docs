...
Note that you may want to generate more than one keypair. For example, if you'd like to run a block producer most securely, you'll want a <DocLink copy="hot wallet / cold wallet block production setup" url="/advanced/hot-cold-block-production" />.
...
See our <DocLink copy="client SDK documentation" url="/using-mina/keypair#client-sdk" /> for more information.
...
If you are using macOS please install Docker and follow the docker based instructions bellow.
...
Check that keygen tool installed correctly by running `mina-generate-keypair -version`. The output should read `Commit a42bdeef6b0c15ee34616e4df76c882b0c5c7c2a on branch master
`.
...
```
docker run --interactive --tty --rm --entrypoint=mina-validate-keypair --volume $(pwd)/keys:/keys minaprotocol/generate-keypair:1.1.5-a42bdee -privkey-path /keys/my-wallet
```

## Next steps

Now that we have created our keypair, we can finally <DocLink copy="connect to the network" url="/using-mina/connecting" /> or share your public key.
