...
sudo apt-get update
sudo apt-get install mina-generate-keypair=1.2.1-45440dc
```

Check that keygen tool installed correctly by running `mina-generate-keypair -version`. The output should read `Commit 45440dc7e4c5dac480dd075357c1d369b893fbe1 on branch master `.
...
```
cd ~
docker run  --interactive --tty --rm --volume $(pwd)/keys:/keys minaprotocol/mina-generate-keypair:1.2.1-45440dc --privkey-path /keys/my-wallet
```
