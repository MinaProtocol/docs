...
sudo apt-get update
sudo apt-get install mina-generate-keypair=1.3.0-c278209
```

Check that keygen tool installed correctly by running `mina-generate-keypair -version`. The output should read `Commit c27820915d358366f027f025030e2d2d158c4d53 on branch master `.
...
```
cd ~
docker run  --interactive --tty --rm --volume $(pwd)/keys:/keys minaprotocol/mina-generate-keypair:1.3.0-c278209 --privkey-path /keys/my-wallet
```
