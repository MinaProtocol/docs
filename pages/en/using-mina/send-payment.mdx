...
If you want to send a transaction without running a node yourself, but by delegating to someone else running anode, keep following along here. If you wish to send the transaction directly with a running node, skip to [using a connected node](#using-a-connected-node).
...
Coming soon a better tool -- see https://github.com/MinaProtocol/mina/issues/8928 , for now, please use the workaround provided in a comment on that issue.
