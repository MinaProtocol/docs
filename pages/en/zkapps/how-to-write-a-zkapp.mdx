...
4. **Deploy to Testnet:** Run `zk config`. This will create a configuration file used to deploy. Run `zk deploy`. This will use your config file and deploy your smart contract. <DocLink copy="See our Deployment section for more details." url="/zkapps/how-to-write-a-zkapp#deployment" />
...
It will ask you to specify a name (can be anything) and URL to deploy to. The URL is the Mina GraphQL API that will receive your deploy transaction and broadcast it to the Mina network. Note that this URL is important because it also determines which network you're be deploying to (e.g. Testnet, Mainnet, etc)

When running the config command, you will need to specify a network name and a URL to deploy too. The network name can be anything you want but in this example, set it to `berkeley`. The URL will be the endpoint used to broadcast your zkApp deployment to the network, please specify `https://proxy.berkeley.minaexplorer.com/graphql` as the URL. The fee is the fee amount in MINA that will be used when deploying to this network alias.
...
<Alert kind="info">
  If your project contains multiple smart contracts (e.g. Foo and Bar) that you
  intend to deploy to the same network, we recommend following a naming pattern
  such as berkeley-foo and berkeley-bar for your network alias names.
</Alert>
