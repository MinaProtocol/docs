...
Custom tokens are a way to create new types of tokens that can be used in the zkApp ecosystem. Custom tokens are identified with a unique "token id" assigned to every kind of token when they are newly minted.

Custom tokens are typically governed by zkApp accounts, where zkApp accounts can mint/burn tokens and facilitate transfers of two separate accounts. zkApps developers can use the supported SnarkyJS API for custom tokens to send, burn, and authorize transactions between other accounts.

##### Custom Token API

There are three main methods for zkApps to interact with custom tokens. This section will discuss how minting, burning, and sending custom tokens are exposed to zkApp developers.
...
Burning tokens is the opposite of minting. Burning tokens deductes the balance of a certain address by the specified amount. See the example below of how a zkApp can burn tokens of another account:

```ts
class BurnExample extends SmartContract {
...
@method burnTokens(receiverAddress: PublicKey) {
    this.token().burn({
      address: receiverAddress,
      amount: 1000000,
    });
...
**Note**: A zkApp cannot burn more tokens than the specified account has. If there is such a case, an error is thrown and no such transaction is made.

##### Sending

If two accounts hold a balance in a custom token, a zkApp must authorize such transaction. Sending tokens requires authorization from the zkApp, which is done by calling the `send` method on the `token` property. See the example below of how a zkApp can authorize sending tokens between two accounts:
...
In the snippet above, we define a smart contract called `SendExample` with a method called `sendTokens`. In the same fashion as minting and burning, we use the `this.token()` property to call the `send` method.

By defining a send method on our smart contract, the zkApp will fill in the authorization it needs to faciltate the transfer between two seperate accounts. Once that has been supplied, the two accounts simply add/subtract the specified amount from their respective balances.

##### Understanding Important Terms

If you are writing your own zkApp to interact with custom tokens, you should understand the following important terms:

##### Token Accounts

Token accounts are like regular accounts, but they hold a balance of a specific custom token instead of MINA. They are created from existing accounts and are specified by a public key and token id.
...
To create a token account for a specified custom token, whenever an existing account receives a transaction denoted with a custom token, a token account is automatically created for that public key.
...
A token owner is an account that facilitates and governs how a custom token is to be used. Custom tokens are not created by sending a transaction to the network, but rather they are _derived_ from an existing account.
