...
1. Install <a
   ...
   zkApp will use then this object to interact with the wallet.
...
// TODO Yoni: Retina Screenshot (`abc123...vxyz <dot>`)
...
```ts
// TODO DISCUSS: Maybe show one line calling smart contract prover method if
// it's short, to show how things connect.
try {
  const { txId } = await window.mina.sendTransaction({
    // fee: 1, // ?
    to: "B62qm3V7qSQ975tgo2MXv4Bpof2cVezZyP1VZJ8DfrVNy9yfqKZxLQf",
    memo: "", // optional
  });
  console.log(txId);
} catch (err) {
...
<Alert kind="info">
If an error occurs when sending the transaction, you may to show the error
