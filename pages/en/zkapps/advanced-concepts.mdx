...
Let's see how `assertBetween` can be used in the voting example mentioned earlier. Assume we want to allow voting throughout September 2022. Timestamps are represented as a `UInt64` and counted in epoch milliseconds; the JS `Date` object makes it easy for us to convert to this representation. In the simplest case, our zkApp could just hard-code the dates:

```ts
let startDate = UInt64.from(Date.UTC(2022, 9, 1));
let endDate = UInt64.from(Date.UTC(2022, 10, 1));
...
In addition to using a predefined range, you can also construct a range that depends on another variable, such as the current time. For example, a DEX with an order book could support orders that expire after an hour. So, the range should be `[now, now + 1h]`. This works just as you'd expect:
...
For completenes, below is the full list of network states you can use and make assertions about in your zkApp.

All of those fields have a `get` and an `assertEquals` method, and the subset that represent
"ordered values" also have `assertBetween` (they are the ones that are `UInt32` or `UInt64`).

Of course, there's no need to remember this -- just type "this.network." and let IntelliSense guide you!

```ts
// current time, as measured by the block producer
...
If you try this, you'll see that SnarkyJS throws a helpful error pointing you to `assertEquals` and `asserBetween`.
...
there's `assertNothing` on all of these fields (including on-chain state).
