...
More formally, you want to ensure that your state updates obey the following commutattivity law (given a left-associative update $\circ$):
...
We also have a [full code example](https://github.com/o1-labs/snarkyjs/blob/main/src/examples/state_update_rollup.ts) which demonstrates this pattern. Leveraging `Reducer.reduce`, it takes only about 30 lines of code to build a zkApp which handles concurrent state updates.
