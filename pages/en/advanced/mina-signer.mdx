...
This is a NodeJS SDK that allows you to generate Mina key pairs and sign transactions and strings using Mina's key pairs.
The project contains Typescript but can be used from plain NodeJS as well.
...
The source code for our SDK is open source and be seen [here](https://github.com/MinaProtocol/mina/tree/develop/frontend/mina-signer).

To get the SDK installed, you will need NodeJS on your system. Make sure it is available on your machine and then use either yarn or npm to install the Mina Signer.
...
Creating a key pair is simple with the Mina Signer. See the snippet below to see how to get create a key pair.
...
## Import your account generated from the Mina Signer

Please note that there are differences in key pair format between what the Mina client expects and what the Mina Signer generates.
When the Mina Signer is used to generate a key pair, the key pair output returned are strings that aren't directly importable into our Mina client.
...
7. Finally, we can import the account created by the Mina Signer into the client.
