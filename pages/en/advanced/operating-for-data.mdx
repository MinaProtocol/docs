...
In order to expose recent information of the blockchain, we'll need to expose the GraphQL REST server. You may want to provide `-rest-server-port XXXX` if the default of 0xc0d or 3085 does not work for you. You may also want to provide `-insecure-rest-server` to make the REST server accessable from outside of `localhost`. However, if you do, make sure you set your firewall up properly. See <DocLink copy="GraphQL API page" url="/developers/graphql-api" /> for more.
