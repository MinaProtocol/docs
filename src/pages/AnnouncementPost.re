...
[@react.component]
let make = (~post: option(ContentType.Announcement.t)) => {
  switch (post) {
  | None =>
    <Page title="Mina Protocol Announcements">
      <div> {React.string("Couldn't find that announcement!")} </div>
      <Next.Link href="/announcements">
        <a> {React.string("Check out the rest of our posts instead")} </a>
      </Next.Link>
    </Page>
  | Some(({title, date, text, snippet, slug}: ContentType.Announcement.t)) =>
    // Manually set the canonical route to remove .html
    <Page title description=snippet route={"/announcements/" ++ slug}>
      <Next.Head> Markdown.katexStylesheet </Next.Head>
      <Hero
        metadata={
          Some(
            CategoryDateSourceText.{
              category: "Post",
              date,
              source: "Announcement",
            },
      ...
      />
      <div className=Style.container> <Markdown content=text /> </div>
    </Page>
  };
};

let cache: Js.Dict.t(option(ContentType.Announcement.t)) = Js.Dict.empty();
